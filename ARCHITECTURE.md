...
The same rule binds `map-gen` and `map-gen2`, and there it is not a sentence but something that
fails: `cargo xtask doc-check` reads the three manifests and refuses a workspace in which `sim-core`,
`sim-data` or `sim-replay` names either. A generator the simulation could reach would be a frozen
part of the determinism contract, and every improvement to it would move every recorded hash.
...
| `map-gen` | **yes** | Draws a map from a seed, from layered noise. Pure arithmetic: it touches no disk, and nothing the simulation loads may depend on it. |
| `map-gen2` | **yes** | Draws a map from a seeded list of mountain/land/sea sources that compete for tiles and then shape their own ground. `map-gen`'s sibling, built from a different algorithm; the same promises bind it. |
| `xtask` | **yes** | Headless runner: `run`, `record`, `regen-expected`, `bench`, `gen-map`, `gen-map2`, `doc-check`. |
