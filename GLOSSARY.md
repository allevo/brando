...
| **slope** | The difference between the highest and the lowest ground height across the tiles a building sits on. Zero means level ground. It is worked out when it is needed and never stored, so it cannot disagree with the heights it comes from. It has **one** meaning: what the map generator measures between a tile and the one beside it is *the step to the steepest neighbour*, which is a different quantity over a different set of tiles and is never called a slope. |
| **noise** | Numbers that look random but vary smoothly, so that nearby places get nearby values. It is what a landscape is drawn from: pure randomness would give every tile its own height and no hills at all. |
| **layer** | One round of noise, added on top of the last. Each layer is half the strength and twice the detail of the one before it, so the big layers make the landforms and the small ones the roughness. |
| **ease** | The curve used between two known values instead of a straight line. It is flat at both ends, so where two stretches meet there is no crease — which is why a landscape drawn with it has slopes rather than creases lined up with the axes. |
| **source** | One of the points `map-gen2` grows a map from. A tile becomes the source's terrain when the source's territory reaches it, and — for a mountain or a sea — the source goes on shaping the tile's height afterwards. |
| **strength** | How often a source in `map-gen2` gets a turn, relative to the others: the strongest source in the list acts every round, a weaker one only every so many. It says nothing about how far a source reaches or how much it changes a tile once it does act. |
| **round** | One pass of `map-gen2`'s turn-taking loop. It is **not** called a tick: `tick` already means one step of simulated game time (see `calendar`), and this is a second, unrelated counter belonging to a tool outside the simulation — a second meaning for a word that already has one is the same failure `slope`'s entry guards against. |
