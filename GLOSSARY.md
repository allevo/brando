...
| Task ids `NN` and half numbers `NN.N` | — | A task's id is the number its file name opens with, stated again in its front matter and checked against the name. A half number is work falling between two whole phases — `doc-check` compares the parts one at a time, so `14.10` follows `14.9` and `14.5.5` sits between `14.5` and `14.6`. How far the numbers have got is `ROADMAP.md`'s answer and no other file's. |
