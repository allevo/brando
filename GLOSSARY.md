...
| **source** | One of the points `map-gen` grows a map from. A tile becomes the source's terrain when the source's territory reaches it, and — for a mountain or a sea — the source goes on shaping the tile's height afterwards. |
| **strength** | How often a source in `map-gen` gets a turn, relative to the others: the strongest source in the list acts every round, a weaker one only every so many. It says nothing about how far a source reaches or how much it changes a tile once it does act. |
| **round** | One pass of `map-gen`'s turn-taking loop. It is **not** called a tick: `tick` already means one step of simulated game time (see `calendar`), and this is a second, unrelated counter belonging to a tool outside the simulation — a second meaning for a word that already has one is the same failure `slope`'s entry guards against. |
