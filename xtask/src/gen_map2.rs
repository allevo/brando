//! `gen-map2`: draws a map from a seed and a list of sources, says what it
//! drew, and writes it out.
//!
//! Thin on purpose, the same as `gen_map.rs`: the arithmetic belongs to
//! `map-gen2` and the writing to `sim-data`. The one thing this front door
//! owns that `gen_map.rs` does not is the shape a list of sources comes in
//! — either a small RON file (`--sources <path>`) or one `--source
//! <x>,<y>,<kind>,<strength>` flag per source, typed straight on the
//! command line — because nothing else in the tree ever reads or writes
//! either shape, so it belongs here rather than costing `map-gen2` a
//! `serde`/`ron` dependency of its own, the same principle `map-gen`'s own
//! doc comment already states for the output side: the crate that consumes
//! a format for a purpose owns it.
...
pub fn gen_map2(args: &[String]) -> Result<(), String> {
    ...
    let drawn = map_gen2::draw(&map_gen2::DrawInput {
        seed,
        sources,
        width,
        height,
    ...
    print!("{}", map_gen2::report(&drawn, &data));
...
fn read_sources(args: &[String]) -> Result<(Vec<map_gen2::Source>, String), String> {
...
fn read_sources_file(path: &str) -> Result<Vec<map_gen2::Source>, String> {
    let text = std::fs::read_to_string(path).map_err(|e| format!("{path}: {e}"))?;
    let raw: RawSources = ron::from_str(&text).map_err(|e| format!("{path}: {e}"))?;
    Ok(raw
        .sources
        .into_iter()
        .map(|s| map_gen2::Source {
            point: sim_core::TilePos::new(s.x, s.y),
            kind: match s.kind {
                RawSourceKind::Mountain => map_gen2::SourceKind::Mountain,
                RawSourceKind::Land => map_gen2::SourceKind::Land,
                RawSourceKind::Sea => map_gen2::SourceKind::Sea,
            },
            strength: s.strength,
        })
        .collect())
...
fn parse_source(raw: &str) -> Result<map_gen2::Source, String> {
    ...
    let kind = match kind.to_ascii_lowercase().as_str() {
        "mountain" => map_gen2::SourceKind::Mountain,
        "land" => map_gen2::SourceKind::Land,
        "sea" => map_gen2::SourceKind::Sea,
        _ => {
            return Err(format!(
...
/// being read as a contract — the same device `gen_map.rs`'s own `note`
/// uses, for the same reason. `sources_flags` is whichever of `--sources
/// <path>` or the `--source ...` flags actually named the sources, so the
/// line records the run that really happened.
fn note(seed: u64, sources_flags: &str, id: &str) -> String {
    format!(
        "Drawn by `cargo xtask gen-map2 --seed {seed} {sources_flags} --id {id}`,\n\
...
/// A required flag, read as the number the argument really is — the same
/// helper `gen_map.rs` defines for itself.
...
mod tests {
    ...
    fn a_source_parses_its_four_fields() {
        ...
        assert_eq!(s.kind, map_gen2::SourceKind::Sea);
    ...
    fn kind_is_read_case_insensitively() {
        for text in ["mountain", "Mountain", "MOUNTAIN", "MoUnTaIn"] {
            ...
            assert_eq!(s.kind, map_gen2::SourceKind::Mountain, "{text}");
        }
    }
    ...
    fn source_flags_alone_are_read_into_the_same_shape_a_file_would_give() {
        ...
        assert_eq!(sources[0].kind, map_gen2::SourceKind::Mountain);
        assert_eq!(sources[1].kind, map_gen2::SourceKind::Sea);
