...
pub fn draw(input: &DrawInput) -> Result<Drawn, DrawError> {
    ...
    // can never be 0 for a non-empty source list — unlike a noise-based
    // generator, which really can submerge an entire seed; this is a
    // property of the growth rule, not something every generator gets for
    // free.
    debug_assert!(
