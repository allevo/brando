...
> points there. The sentence is left as it was written.
>
...
>    A12, **the services chase the population**, with a recomputation every
>    tick and its price (~13× on the empty tick). That is where A17 comes
>    from, the first decision in this project to stay open — and, after the bug hunt that follows
>    phase 13, A18 as well.
...
1. **House levels.** `levels` in the tables becomes > 1; service requirements per level. Houses
   ...
   after M0: see A5. The shape of satisfaction (an accumulator of time per
   service, with hysteresis, **not** a resource level) is decided in A10. This
...
2. **Migration.** The first real use of `RngDomain::Migration` — and it is the tick on which test 7
   ...
   (A10), because immigrants as real walkers are M3 (D3).
...
One thing only, and it is A17: recomputing the coverage every tick, which is
the price of A12. The typical tick goes from 248 µs to ~3.3 ms, and the cost is
...
This is where A5 has to be revisited: the farm as a coverage provider is an M0
...
- ~~Is step 3 (coverage) really the hot path, or is it the network rebuild?~~ **Answered:** step 3,
  ...
  (A11). The number to watch now is `A`, the empty tick.
- ~~Does service capacity make sense in "houses served" or "residents served"?~~ **Answered:** in
  ...
  what its output sustains. Details in A5.
- Does 30 ticks/month (A6) give a playable curve? The first scenario with a
  deadline will say.
