...
1. **House levels.** `levels` in the tables becomes > 1; service requirements per level. Houses
   ...
   after M0: see [A5](open-decisions.md). The shape of satisfaction (an accumulator of time per
   service, with hysteresis, **not** a resource level) is decided in [A10](open-decisions.md). This
   is where `required_services` stops being declarative data and starts being read
   ([A9](open-decisions.md)).
2. **Migration.** The first real use of `RngDomain::Migration` — and it is the tick on which test 7
   ...
   ([A10](open-decisions.md)), because immigrants as real walkers are M3 (D3).
...
One thing only, and it is [A17](open-decisions.md): recomputing the coverage every tick, which is
the price of [A12](open-decisions.md). The typical tick goes from 248 µs to ~3.3 ms, and the cost is
...
This is where [A5](open-decisions.md) has to be revisited: the farm as a coverage provider is an M0
...
- ~~Is step 3 (coverage) really the hot path, or is it the network rebuild?~~ **Answered:** step 3,
  ...
  ([A11](open-decisions.md)). The number to watch now is `A`, the empty tick.
- ~~Does service capacity make sense in "houses served" or "residents served"?~~ **Answered:** in
  ...
  what its output sustains. Details in [A5](open-decisions.md).
- Does 30 ticks/month ([A6](open-decisions.md)) give a playable curve? The first scenario with a
  deadline will say.
